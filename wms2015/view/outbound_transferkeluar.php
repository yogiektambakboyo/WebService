...
?>
<div class="header">
    <h1 class="page-title">Task Outbound - Transfer Keluar</h1>
</div>
<div class="main-content" style="height:100%">
    ...
    <div class="scrollable-area" style="max-height:350px;overflow:scroll;font-size: 12px;">
        <div class="row" id="outbound_transferkeluar">
            <div class="table-responsive">
                <table class="table table-hover table-bordered tablesorter" style="white-space: nowrap;margin-left: 2%;" id="outbound_transferkeluar_table">
                    <thead>
                    <tr>
                        <th>No</th>
                        <th>Kodenota</th>
                    ...
                    <tbody>
                    <?php
                    $sql="select Kodenota,CONVERT(CHAR(10), Tgl, 120) as Tgl,TransactionType,Keterangan,isHaveRcpt,isClosed,DriverId,DispatchNo from dbo.OutBound_Master WHERE isClosed=:isClose and (Tgl between :tglAwal and :tglAkhir)  and TransactionType='TransferKeluar'  and LEFT(Kodenota,5)=:cabang order by tgl";
                    $sth = $dbh->prepare($sql);
                    $isClosed = 0;
                    ...
                    foreach ($result as $row) {
                        ...
                        if(($row['isHaveRcpt'] == 1)&&($row['isClosed']== 0)){
                            echo '<td><a href="#" onclick="outboundClose(\''.$row['isHaveRcpt'].'\',\''.$row['isClosed'].'\',\''.$row['Kodenota'].'\')" id="outbound_transfer_modal_close"><span class="glyphicon glyphicon-saved"></span> Close</a></td>';
                        }else{
                            echo '<td></td>';
                        }
                        echo '</tr>';
...
<div id="outbound_transferkeluar_confirm" style="display:none; cursor: default" title="Konfirmasi Task Transfer">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Detail Transfer Keluar : <label id="outbound_transfer_modal_lbl_so"></label> </h4>
            </div>
...
<script type="text/javascript">
    ...
    $("#outbound_transferkeluar_table").tablesorter({
    ...
    $("#updateTableOutbound").click(function(){
        ...
        if (($("#outbound_transferkeluar_table tr").length)>3) {
            $.tablesorter.clearTableBody($("#outbound_transferkeluar_table")[0]);
        }
        //$("#outbound_loading").dialog({ dialogClass: 'noTitleStuff' });
        //$(".ui-dialog-titlebar").hide();
        $.get("./taskoutbound/outbound_ajax_transferkeluar.php?isClosed="+$("#outbound_status option:selected").val()+"&tglAwal="+$("#outbound_tglawal").val()+"&tglAkhir="+$("#outbound_tglakhir").val(), function(html) {
            //$("#outbound_loading").dialog("close");
            // append the "ajax'd" data to the table body
            $("#outbound_transferkeluar_table tbody").append(html);
            // let the plugin know that we made a update
            $("#outbound_transferkeluar_table").trigger("update");
            $.unblockUI();
        });
        return false;
    });
    ...
    function showDetail(Kodenota){
        $("#myModal").modal("show");
        var IdSO = Kodenota;
        $("#outbound_transfer_modal_lbl_so").text(IdSO);
    ...
    function outboundClose(isReceipt,isClosed,kodenota){
        if((isReceipt==1)&&(isClosed==0)){
            $.ajax({
                url: "./taskoutbound/outbound_ajax_transfer_close.php?id="+kodenota,
                type: "GET",
                data: {},
                dataType: 'json',
                success: function(data){
