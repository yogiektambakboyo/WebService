...
$sql="select POnumber,Shipto,Keterangan,isHaveRcpt,isClosed from inbound_master
		where tgl between :tglawal and :tglakhir and TransactionType='TransferMasuk' and POnumber like :divisi";
$sth = $dbh->prepare($sql);
$result = $sth->execute(array(
			":tglawal"=>$tglawal,
			":tglakhir"=>$tglakhir,
...
$i++;
		$listquery.='<tr>		
						...
						<td>
						    ...
						    <button type="button" class="btn btn-warning tblbtn closebtn" onclick="checkdivisi(\'view/closetask.php?inbound=TransferMasuk&nopo='.$row['POnumber'].'&shipto='.$row['Shipto'].'\',\'#viewarea\')">Close</button>
...
<div class="table-responsive">
    <table class="table table-hover table-bordered tablesorter" style="white-space: nowrap;" id="RetTable">
        <thead>
        <tr>
            <th>No</th>
            <th>PO Number</th>
