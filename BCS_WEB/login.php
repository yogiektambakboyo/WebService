<?php
session_start();
if ((isset($_SESSION["usernamebcs"]))&&(isset($_SESSION["jabatanbcs"]))&&(isset($_SESSION["cabangbcs"]))) {
...
<head>
    ...
    <title>BCS - Borwita Customer Survey</title>
            ...
            if(isset($_POST["BtnLogin"])){
                ...
                if(($username=="")||($password=="")){
                    ...
                    if ($koneksi["status"]) {
                            ...
                            if($result["jumdata"]>0){
                                $sql = "select kode,nama,jabatan from staff where nama='".$username."' and jabatan in ('ADMINISTRATOR','ASM','SALES')" ;
                                $result = $db->queryDB($sql);
                                if($result["jumdata"]>0){
                                    while ($row = mssql_fetch_assoc($result["result"])) {
                                        ...
                                        $_SESSION["usernamebcs"]=$row["nama"];
                                        $_SESSION["jabatanbcs"]=$row["jabatan"];
                ...
                $("#dialog-confirm").dialog({
                    ...
                    buttons: {
                        "Submit": function () {
                            $(this).dialog('close');
                            $.ajax({
                                url: 'bcs_rest.php?act=gawejejak&cabang='+$('#slcCabangDlg option:selected').val(),
                                type: 'POST',
                                data : {} ,
                                success: function (response) {
                                    window.location = window.location.origin+"/BCS/index.php";
                                },
                                error: function () {}
                            });
                        }
            ...
            <fieldset>
                <div class="input-group">
                    <select id="slcCabangDlg" class="form-control" name="slcCabang">
                        ...
                        $koneksi = $db->connectDB("00");
                        if ($koneksi["status"]) {
                            $sql = "select Kode,Keterangan from cabang where Kode between '01' and '25' order by Kode" ;
