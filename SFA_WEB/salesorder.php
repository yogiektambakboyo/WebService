...
if ($koneksi["status"]) {
    if (isset($_POST["orderbtn"])) {
        ...
        $tglinput=$tgl;
        if ($tgl != "") {
            $sql = "select distinct sales,ISNULL(s.nama,'') as nama,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,m.createdate),0),121) as createdate,d.keterangan as divisi from masterorder m join divisi d on d.kode=substring(sales,4,2) left join salesperson s on s.kode=m.sales where tgl='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
    ...
    if (isset($_POST["uploadbtn"])) {
        ...
        $tglinput=$tgl;
        if ($tgl != "") {
            $sql = "select distinct sales,ISNULL(s.nama,'') as nama,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,m.createdate),0),121) as createdate,d.keterangan as divisi from masterorder m join divisi d on d.kode=substring(sales,4,2) left join salesperson s on s.kode=m.sales where CONVERT(VARCHAR(10), m.createdate, 121)='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
            $result = $db->queryDB($sql);
            if ($result["jumdata"] == 0) {
                $arr = array("status" => 0, "data" => "Data Kosong");
            }
        ...
        if (count($order) > 0) {
            ...
            if($validate==1){
                ...
                foreach ($order as $roworder) {
                    ...
                    $sales = $odr[0];
                    $tgl = $odr[1];
                    $sql = "select * from masterorder where sales='" . $sales . "' and tgl='".$tgl."'";
                    ...
                    if ($result["jumdata"] > 0) {
                        while ($row = mssql_fetch_assoc($result["result"])) {
                            ...
                            <tr>
                                ...
                                <td><input id="<?php echo "show".strrev(str_replace('/','',$row['sales'])) ?>" type="button" class="btn-small btn-info" onclick="<?php echo "append('".str_replace('/','',strrev($row['sales']))."',".date('Ymd', strtotime($row['tgl'])).")"; ?>" value="Detail"><input hidden="true" id="<?php echo "hide".strrev(str_replace('/','',$row['sales'])) ?>" type="button" class="btn-small btn-warning"  onclick="<?php echo "RemoveTR('".str_replace('/','',strrev($row['sales']))."')"; ?>" value="Close"></td>
                            </tr>
                        <?php
                        }
                    }
...
$('#selectall').click(function(){
    if($('#selectall').html()=='Select All')
    {
        ...
        if($('#divisi').val()=='01'){
            $('input:checkbox[id^="P&G"]').each(function(){
        ...
        if($('#divisi').val()=='13'){
            $('input:checkbox[id^="PTPN 8"]').each(function(){
                $(this).prop('checked', true);
            });
        }
        if($('#divisi').val()=='14'){
            $('input:checkbox[id^="TOMORION"]').each(function(){
                $(this).prop('checked', true);
            });
        }
        if($('#divisi').val()=='15'){
            $('input:checkbox[id^="OMEGA MIX"]').each(function(){
                $(this).prop('checked', true);
            });
        }
    ...
    if(mydiv.indexOf("PTPN 8") > -1){
        $('#divisi').val("13");
    }
    if(mydiv.indexOf("TOMORION") > -1){
        $('#divisi').val("14");
    }
    if(mydiv.indexOf("OMEGA MIX") > -1){
        $('#divisi').val("15");
    }
        ...
        success: function(data){
            if(data.status==1){
                ...
                $.each(data.data, function(i,item){
                    str+='<tr id="detailinv'+sales+'"><td>'+item["nama"]+'</td><td>'+item["noorder"]+'</td><td>'+item["shipto"]+'</td><td><input type="button" class="btn-small btn-info" onclick="getAjaxBarang(\''+item["noorder"].substring(0,8).split("").reverse().join("")+'\','+item["noorder"].substring(9,15)+',\''+item["noorder"].substring(16,19).split("").reverse().join("")+'\')" value="Detail"></td></tr>';
                });
