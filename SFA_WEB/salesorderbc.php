...
if ($koneksi["status"]) {
    if (isset($_POST["orderbtn"])) {
        ...
        if ($tgl != "") {
            //$sql = "select distinct sales,nama,noorder,shipto,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,createdate),0),121) as createdate from masterorder where tgl='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
            $sql = "select distinct sales,ISNULL(s.nama,'') as nama,noorder,shipto,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,m.createdate),0),121) as createdate,d.keterangan as divisi from masterorder m join divisi d on d.kode=substring(sales,4,2) left join salesperson s on s.kode=m.sales where tgl='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
    ...
    if (isset($_POST["uploadbtn"])) {
        ...
        if ($tgl != "") {
            //$sql = "select distinct sales,nama,noorder,shipto,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,createdate),0),121) as createdate from masterorder where CONVERT(VARCHAR(10), createdate, 121)='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
            $sql = "select distinct sales,ISNULL(s.nama,'') as nama,noorder,shipto,tgl,CONVERT(VARCHAR(16),dateadd(day,datediff(day,0,m.createdate),0),121) as createdate,d.keterangan as divisi from masterorder m join divisi d on d.kode=substring(sales,4,2) left join salesperson s on s.kode=m.sales where tgl='" . $tgl . "' and left(sales,2)='".$_SESSION["cabang"]."'";
            $result = $db->queryDB($sql);
            if ($result["jumdata"] == 0) {
                $arr = array("status" => 0, "data" => "Data Kosong");
    ...
    if (isset($_POST["csvexport"])) {
        ...
        $divisi = $_POST["divisi"];
        if (count($order) > 0) {
            $title = "OrderEntry_" . $_SESSION["cabang"].$divisi . "_" . date("Ymd").".csv";
            ...
            foreach ($order as $roworder) {
                $sql = "select * from masterorder where noorder='" . $roworder . "'";
    ...
    <body>
                            ...
                            <tbody>
                                ...
                                if ($result["jumdata"] > 0) {
                                    while ($row = mssql_fetch_assoc($result["result"])) {
                                        ?>
                                        <tr>
                                            <td><input type="checkbox" name="order[]" value="<?php echo $row['noorder'] ?>" id="<?php echo $row['divisi'] ?>" /></td>
                                            <td id="mydivisi"><?php echo $row["divisi"] ?></td>
                                            <td><?php echo $row["sales"] ?></td>
                                            <td><?php echo $row["nama"] ?></td>
                                            <td id="noorder"><?php echo $row["noorder"] ?></td>
        ...
        <script type="text/javascript">
            ...
            $('#tableorder tbody tr').click(function(event) {
                ...
                var noorder=$(this).find("#noorder").text();
                var link = "get_ajax_barang.php";
                var postdata={'noorder':noorder};
                $.ajax({
                    type: 'POST',
                    url: link,
                    dataType: 'jsonp',
                    ...
                    success: function(data){
                        if(data.status==1){
                            ...
                            $.each(data.data, function(i,item){
                                str+='<tr><td>'+item["brg"]+'</td><td>'+item["keterangan"]+'</td><td>'+item["crt"]+'</td><td>'+item["pcs"]+'</td></tr>';
                            });
