...
if($koneksi["status"]){
    ...
    //select barang
    $sql="select b.Kode,b.Keterangan as Nama,ISNULL(b.Hint,'Data Kosong') as Keterangan,bm.Keterangan as Merek,bv.Keterangan as Variant,s.CRT,ISNULL(dbo.hrgjualbarang(b.kode,getdate(),1,'',''),0) Harga
            ...
            and s.brg=b.kode order by b.Keterangan";
    ...
    if($result2["jumdata"]==0){
        ...
        if($status4){
            ...
            $resultcabang[] = array("status"=>1,"data"=>"Data Bisa Download","minorder"=>$minOrder);
        }else{
            $arr=array("status"=>0,"data"=>"FJP ".$sqlite->lastErrorMsg());
        }
    }
