...
if (isset($_POST["btnSync"])) {
    ...
    if ($koneksibrg["status"]) {
        ...
        $dbmaster = new DB();
        $koneksimaster = $dbmaster->connectDB("00");
        if ($koneksimaster["status"]) {
            $statusrfs = true;
            foreach ($barangfrom as $row) {
                $sql = "select * from barang where kode='" . $row["kode"] . "'";
                $brg = $dbmaster->queryDB($sql);
                if ($brg["jumdata"] > 0) {
                    $sql = "update barang set keterangan='" . $row["keterangan"] . "',hint='" . $row["hint"] . "' where kode='" . $row["kode"] . "'";
                } else {
                    $sql = "insert into barang values('" . $row["kode"] . "','" . $row["keterangan"] . "','" . $row["hint"] . "')";
                }
