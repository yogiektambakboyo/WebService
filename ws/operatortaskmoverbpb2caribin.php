...
if($koneksi["status"]){
    $sql = "SELECT DISTINCT h.NoUrut,h.QueueNumber,m.ERPCode,h.QueueNumber,h.TransactionCode,h.BinCode,h.SKUCode,b.Keterangan,h.Qty,dbo.KonversiSatuanToText(h.SKUCode,h.Qty) as QtyKonversi,de.Ratio,de.RatioName,
                ...
                AND h.BinCode='". $BinCode ."'
                ORDER BY h.QueueNumber DESC";
    $result=$db->queryDB($sql);

    if($result["jumdata"]>0){
        while ($row = mssql_fetch_assoc($result["result"])) {$resultcabang[]=$row;}
    }else{$resultcabang[]=array("NoUrut"=>"0","QueueNumber"=>"0","ERPCode"=>"Data Kosong","TransactionCode"=>"0","BinCode"=>"0","SKUCode"=>"0","Keterangan"=>"0","Qty"=>"0","QtyKonversi"=>"0","Ratio"=>"0","RatioName"=>"0","DestRackSlot"=>"0","CurrRackSlot"=>"0");}
}
else{$resultcabang[]=array("NoUrut"=>"0","QueueNumber"=>"0","ERPCode"=>"Koneksi Server terputus","TransactionCode"=>"0","BinCode"=>"0","SKUCode"=>"0","Keterangan"=>"0","Qty"=>"0","QtyKonversi"=>"0","Ratio"=>"0","RatioName"=>"0","DestRackSlot"=>"0","CurrRackSlot"=>"0");}
