...
class DB{
    ...
    public function connectDB($cabang){
        ...
        if (!$this->server) {$data["status"]=false;$data["data"]="Koneksi Server Gagal";}else{
    ...
    public function queryDB($sql){$result = mssql_query($sql);$data["jumdata"] = mssql_num_rows($result);$data["result"]=$result;return $data;}
    public function executeDB($sql){$result = mssql_query($sql);return $result;}
