 ...
 * @author USER
 */
class admin_picking extends CI_Controller {
     ...
     function __construct() {
        ...
        $this->load->model('admin_picking_model');
    }

    function index() {
        $this->load->view('admin_picking/admin_picking_main_view');
    }
    ...
    function tambahpicklist() {
        if ($this->input->post('btnProses')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                $data['cabang'] = $this->admin_picking_model->getCabang();
                ...
                $data['picklist'] = $this->admin_picking_model->getPickingList($today, $defaultcabang);
                $data['defaultcabang']=$defaultcabang;
                $this->load->view('admin_picking/tambah_picklist_view', $data);
            } else {
                ...
                $kode_proyek = 'PCK'; //kode Picking  
                $str="";
                foreach ($picklist as $row) {
                    $str.=$row.";";
                }
                $str=  substr($str, 0,-1);
                $this->admin_picking_model->setPicklist($kode_proyek, $str, $tanggal, $siteId, $operatorCode);
            ...
            $data['tgl']=$today;
            $data['picklist'] = $this->admin_picking_model->getPickingList($today, $cabang);
            $this->load->view('admin_picking/tambah_picklist_view', $data);
        }
    }
    ...
    function get_ajax_tambahpicklist() {
        ...
        $result = $this->admin_picking_model->getPickingList($tgl, $cabang);
        $arr = array();
        foreach ($result as $row) {
            $arr[] = array("NoPicklist" => $row['NoPicklist'], "TglPicklist" => date('d-m-Y', strtotime($row['TglPicklist'])), "JmlInv" => $row['JmlInv'],"keterangan"=>$row['keterangan']);
        }
        echo $_GET['jsoncallback'] . '(' . json_encode($arr) . ');';
    }

    function daftar_task_picking() {
        $data['masterpck'] = $this->admin_picking_model->getMasterTaskPicking();
        $this->load->view('admin_picking/daftar_task_pck_view', $data);
    }
    ...
    function edit_mastertaskpck($TransactionCode) {
        ...
        $result = $this->admin_picking_model->getstatustask($TransactionCode);
        if ($result['isCancel'] == 0) {
            $status = $this->admin_picking_model->getstatustask($TransactionCode);
            //$this->session->set_userdata('TransactionCode', $TransactionCode);
            $data['isFinishMove'] = $result['isFinishMove'];
            $data['note'] = $this->admin_picking_model->getNoteMaster($TransactionCode);
            $this->load->view('admin_picking/edit_note_master_view', $data);
        }
    }

    function detail_transaksi_pck($transactionCode) {
        $transactionCode = base64_decode(str_replace('-', '=', str_replace('_', '/', $transactionCode)));
        $data['transaksi'] = $this->admin_picking_model->getDetailTransaction($transactionCode);
        $data['detail_transaksi'] = $this->admin_picking_model->getDetailTransaction2($transactionCode);
        $this->load->view('admin_picking/daftar_detail_transaksi_pck_view', $data);
    }
    ...
    function simpan_mastertaskpck() {
        if ($this->input->post('btnSimpan')) {
            ...
            $operatorCode = $this->session->userdata('OperatorCode');
            $tanggal = date("Y/m/d");
            $kode_proyek = 'PCK'; //kode BPB Principle    

            $this->admin_picking_model->setPicklist($kode_proyek, $NoPickList, $tanggal, $siteId, $operatorCode);
            $this->admin_picking_model->setNoteMaster($TransactionCode, $Note);
    ...
    function edit_note($TransactionCode, $NoUrut, $Status2) {
        ...
        $data['note'] = $this->admin_picking_model->getNote($TransactionCode, $NoUrut);
        $data['Status2'] = $Status2;
        $this->load->view('admin_picking/edit_note_view', $data);
    }
    ...
    function simpan_note() {
        if ($this->input->post('btnSimpan')) {
            ...
            if ($this->admin_picking_model->setNote($TransactionCode, $NoUrut, $Note)) {
                $data['transaksi'] = $this->admin_picking_model->getDetailTransaction($TransactionCode);
                $data['detail_transaksi'] = $this->admin_picking_model->getDetailTransaction2($TransactionCode);
                $this->load->view('admin_picking/daftar_detail_transaksi_pck_view', $data);
            } else {
    ...
    function simpan_pembatalanpck() {
        if ($this->input->post('btnSimpanCancel')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $tanggal = date("Y/m/d");
                $kode_proyek = 'PCK'; //kode BPB Principle 
                $Note = $this->admin_picking_model->getNoteMaster($TransactionCode);
                $Notelama = $Note['Note'];
                ...
                $this->admin_picking_model->setNoteMasterPembatalan($TransactionCode, $Notelama, $operatorCode);
                $this->daftar_task_picking();
            }
        }
    }
    ...
    function cek_finishmove($TransactionCode, $ERPCode) {
        ...
        $OutstandingMove = $this->admin_picking_model->cekOutstandingFinishMove($TransactionCode);
        if (count($OutstandingMove) > 0) {
            ...
            $this->load->view('admin_picking/outstanding_finishmove_view', $data);
        } else {
            $OperatorCode = $this->session->userdata('OperatorCode');
            if ($this->admin_picking_model->setFinishMove($TransactionCode, $OperatorCode)) {
    ...
    function cek_finishadmin($TransactionCode, $ERPCode) {
        $TransactionCode = base64_decode(str_replace('-', '=', str_replace('_', '/', $TransactionCode)));
        $ERPCode = base64_decode(str_replace('-', '=', str_replace('_', '/', $ERPCode)));
        $OutstandingDERP = $this->admin_picking_model->cekOutstandingFinishAdmin($TransactionCode);
        if (count($OutstandingDERP) > 0) {
            $data['outstanding'] = $OutstandingDERP;
            $data['TransactionCode'] = $TransactionCode;
            $data['penyelesaian'] = $this->admin_picking_model->getNotaPenyelesaian($TransactionCode);
            $this->session->set_userdata('TransactionCode', $TransactionCode);
            $this->session->set_userdata('ERPCode', $ERPCode);
            $data['ERPCode'] = $ERPCode;
            $this->load->view('admin_picking/outstanding_finishadmin_view', $data);
        } else {
            $OperatorCode = $this->session->userdata('OperatorCode');
            if ($this->admin_picking_model->setFinishAdmin($TransactionCode, $OperatorCode)) {
    ...
    function cek_detailpembatalan($kode) {
        if ($this->admin_picking_model->cekPembatalan($kode)) {
            return TRUE;
        }
    ...
    function tambah_notapenyelesaian() {
        if ($this->input->post('btnSimpan')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $OutstandingDERP = $this->admin_picking_model->cekOutstandingFinishAdmin($data['TransactionCode']);
                $data['outstanding'] = $OutstandingDERP;

                $data['penyelesaian'] = $this->admin_picking_model->getNotaPenyelesaian($data['TransactionCode']);
                ...
                if ($this->admin_picking_model->setPenyelesaian($TransactionCode, $KodeNota, $Tipe)) {
    ...
    function hapus_kodenota($TransactionCode, $KodeNota) {
        ...
        if ($this->admin_picking_model->removePenyelesaian($TransactionCode, $KodeNota)) {
    ...
    function cek_kodenota($KodeNota) {
        if ($this->admin_picking_model->cekKodeNota($KodeNota, $this->session->userdata('ERPCode'))) {
            return true;
        }
	...
	{	
		if ($this->input->post('btnYa')){
			...
			$this->admin_picking_model->hapusPickingAll($tr);
			$this->session->set_flashdata('success','Picklist Berhasil Dihapus') ;
			redirect('admin_picking/list_picking');
		}
		
		$data['cabang']	  = $this->admin_picking_model->getCabang($this->session->userdata('HariCabang'));
		$data['picklist'] = $this->admin_picking_model->getListEditPicklist();
		$this->load->view('admin_picking/list_picking', $data);
	}
	...
	public function edit_picking($tr)
	{
		...
		if ($this->input->post('btnYa')){
			...
			$cek = $this->admin_picking_model->getEdit($transactionCode);
			// var_dump(count($cek));
			if (count($cek) <= '1') {
				$this->session->set_flashdata('error','Picklist Gagal Dihapus') ;
				redirect('admin_picking/list_picking');
			}else{
				$this->admin_picking_model->hapusPicking($tr,$pl);
				$data['sucses'] = 'Picklist Berhasil Dihapus' ;
			}
		}
		
		$data['edit'] = $this->admin_picking_model->getEdit($transactionCode);
		$this->load->view('admin_picking/edit_picking',$data);
	}
	...
	public function edit_picking2($tr)
	{
		...
		$this->load->view('admin_picking/edit_picking2ajax',$data);
	}
    public function show_assigned($TransactionCode, $ERPCode) {
        ...
        $this->session->set_userdata('ERPCode', $ERPCode);
        $this->session->set_userdata('TransactionCode', $TransactionCode);
        $data['Assigned'] = $this->admin_picking_model->getAssigned($TransactionCode);
        $data['Role'] = $this->admin_picking_model->getRole($TransactionCode);
        $this->load->view('admin_picking/Assigned_view', $data);
    }
    ...
    public function delete_assigned($OperatorCode, $OprRole) {
        ...
        if ($this->admin_picking_model->deleteAssigned($TransactionCode, $OperatorCode, $OprRole)) {
            $data['Assigned'] = $this->admin_picking_model->getAssigned($TransactionCode);
            $data['Role'] = $this->admin_picking_model->getRole($TransactionCode);
    ...
    function get_ajax_Operator() {
        ...
        $TransactionCode = $this->session->userdata('TransactionCode');
        $result = $this->admin_picking_model->getOperator($WHRoleCode, $TransactionCode);
    ...
    function tambah_assigned() {
        if ($this->input->post('btnTambah')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->admin_picking_model->setAssigned($TransactionCode, $OperatorCode, $OprRole)) {
                    $data['Assigned'] = $this->admin_picking_model->getAssigned($TransactionCode);
                    $data['Role'] = $this->admin_picking_model->getRole($TransactionCode);
