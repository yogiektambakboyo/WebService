...
class admin_replenish_model extends CI_Model {
    ...
    function getBarang() {
        $sql = "select b.Kode,b.Keterangan,dbo.KonversiSatuanToText(b.Kode,bs.Qty) as Qtykonversi,bs.Qty,bi.WHCode,g.Keterangan as WHName,r.Name as 		RackName,bs.ExpDate,bi.RackSlotCode,bi.BinCode 
           ...
           where bi.WHcode  in (select kode from wms.gudangwms)
            ";
    ...
    function getWH($Rack) {
        $sql = "select count(WHCode) as jumlah from wms.Bin where RackSlotCode='" . $Rack . "' and WHCode in (select kode from wms.gudangwms)";

        $result = $this->db->conn_id->prepare($sql);
        $result->execute();
        $result = $result->fetchAll();
        if($result[0]['jumlah']>0){
            $sql = "select WHCode from wms.Bin where RackSlotCode='" . $Rack . "' and WHCode in (select kode from wms.gudangwms)";
