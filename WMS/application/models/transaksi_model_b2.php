...
class Transaksi_model extends CI_Model {
    ...
    function getTodayBPBList($tgl, $cabang) {
        $sql = "select m.Kodenota,s.Perusahaan,m.Keterangan
                from masterbeli m,supplier s
                where m.kodenota like '" . $cabang . "'+'/B%' and m.tgl='" . $tgl . "'
                and s.kode=m.shipfrom and m.total>0 and not exists
    ...
    function getTodayTransaksiBPBList() {
        ...
        $sql = "select t.TransactionDate,t.isFinishMove,t.transactionCode,t.projectCode,t.note,t.isFinish,t.isCancel,b.kodeNota,s.perusahaan
                from wms.MasterTaskRcv t
                join masterbeli b on b.kodenota=t.ERPCode
                join supplier s on s.kode = b.supplier
    ...
    function getDetailTransaction2($TransactionCode) { //detail transaksi
        $sql = "select (select Name from wms.RackSlot where RackSlotCode=dh.CurrRackSlot) as RackSlotSekarang,(select Name from wms.RackSlot where RackSlotCode=dt.DestRackSlot) as RackSlotTujuan,dt.*,dbo.KonversiSatuanToText(dt.SKUCode,dt.Qty) as Qtykonversi,convert(varchar(10),dt.ExpDate,121) tanggalExp,dt.CreateTime waktuBuat,b.keterangan
                ...
                where dt.TransactionCode =  '" . $TransactionCode . "' and dt.BinCode<>9999999
                and dh.QueueNumber=
    ...
    function setNotePembatalan($TransactionCode, $NoUrut, $QueueNumber, $OperatorCode) {
        ...
        $sql = "update wms.DetailTaskRcvhistory set User_2nd='" . $OperatorCode . "', Time_2nd=getdate(), DestRackSlot=CurrRackSlot, DestOnAisle=0, DestBin='9999999', DestQty=Qty
    ...
    function setOverRcv($TransactionCode,$SKUCode,$Qty){
        $sql = "insert into wms.DetailTaskRcvOver(TransactionCode,SKUCode,Qty) values ('".$TransactionCode."','".$SKUCode."','".$Qty."')";
        $result = $this->db->conn_id->prepare($sql);
        if($result->execute())
        {
            $sql = "update wms.MasterTaskRcv set isFinishMove=0 where TransactionCode='" . $TransactionCode . "'";
