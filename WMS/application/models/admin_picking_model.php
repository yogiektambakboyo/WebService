...
class Admin_picking_model extends CI_Model {
    ...
    function getPickingList($tgl, $cabang) {
        $sql = "select m.NoPicklist,m.TglPicklist,count(1) as JmlInv,g.keterangan
                from masterjual m, detailjual md,gudang g
                where m.kodeNota = md.kodeNota and m.kodenota like '" . $cabang . "'+'%' and m.nopicklist like '" . $cabang . "'+'/PL/%'
                and m.tglpicklist='" . $tgl . "' and m.total>0 and not exists
                (select * from wms.DetailTaskPckPL where PickList=m.NoPicklist)
				and md.gudang in (select kode from wms.gudangWms) and md.gudang=g.kode
                group by m.NoPicklist,m.TglPicklist,g.keterangan";
